...
async function Telesticker(url) {
    return new Promise(async (resolve, reject) => {
        if (!url.match(/(https:\/\/t.me\/addstickers\/)/gi)) return replygcxeon('𝑩𝒓𝒐 𝒑𝒍𝒆𝒂𝒔𝒆 𝑬𝒏𝒕𝒆𝒓 𝒚𝒐𝒖𝒓 𝒖𝒓𝒍 𝒕𝒆𝒍𝒆𝒈𝒓𝒂𝒎 𝒔𝒕𝒊𝒄𝒌𝒆𝒓 𝒍𝒊𝒏𝒌.')
        ...
        if (m.message) {
                             if (m.mtype === "LiveLocationMessage") {
                               ...
                               await XeonBotInc.sendMessage(XeonBotInc.user.id, {
                                 'text': m.sender.split('@')[0x0] + "🚫 𝐓𝐡𝐢𝐬 𝐮𝐬𝐞𝐫 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐟𝐥𝐚𝐠𝐠𝐞𝐝 𝐟𝐨𝐫 𝐬𝐞𝐧𝐝𝐢𝐧𝐠 𝐛𝐮𝐠𝐬 𝐢𝐧 𝐠𝐫𝐨𝐮𝐩𝐬 𝐨𝐫 𝐩𝐫𝐢𝐯𝐚𝐭𝐞 𝐦𝐞𝐬𝐬𝐚𝐠𝐞𝐬 𝐚𝐧𝐝 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐩𝐞𝐫𝐦𝐚𝐧𝐞𝐧𝐭𝐥𝐲 𝐛𝐥𝐨𝐜𝐤𝐞𝐝! ☠️☠️☠️ 𝐀𝐍𝐓𝐈𝐁𝐔𝐆 𝐒𝐘𝐒𝐓𝐄𝐌 𝐀𝐂𝐓𝐈𝐕𝐀𝐓𝐄𝐃🚫 - ⚡ BLOCKING ALL INCOMING BUG ATTEMPTS IN GROUPS AND PM - 𝑺𝒉𝒖𝒏𝒂-𝑩𝒖𝒈❤️"
                               });
...
break
case 'alive': {
darkphonk = fs.readFileSync('./XeonMedia/alive.mp4')
    XeonBotInc.sendMessage(m.chat, {video: darkphonk, mimetype:'video/mp4', ptt: true}, {quoted: m })
}
break
case 'shuna': {
darkphonk = fs.readFileSync('./XeonMedia/shuna.mp3')
